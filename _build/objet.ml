...
let creer_missile_right_obj s text p =
  ...
  {pos= pos; oldpos = pos; speed={vx=12; vy=0}; kind = Missile; w=25; h=10; texture = text;
...
let creer_missile_left_obj s text p =
  ...
  {pos= pos; oldpos = pos; speed={vx= (-12); vy=0}; kind = Missile; w=25; h=10;
...
let creer_bonus x y width height text =
  let pos = {x=x;y=y} in
  let speed = {vx = 0; vy = 0} in
  {pos = pos; oldpos= pos; speed=speed; kind=Bonus; w=width; h=height; texture=text; jumping=false;
   pv = 1; timer_collision = 0; timer_missile = 0; frame = 0; repos = 0; wl =[||]; wr = [||]; jl =[||];
   jr = [||];  s = [||]; m = [||] }
...
let check_collision a b =
  let rectA = Sdl.Rect.create a.pos.x a.pos.y a.w a.h in
  let rectB = Sdl.Rect.create b.pos.x b.pos.y b.w b.h in
